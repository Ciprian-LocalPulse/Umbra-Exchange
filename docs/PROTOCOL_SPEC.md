...
Each contributor holds a keypair issued during onboarding. Tiering (0–3) reflects vetting level — e.g. tier 0 = self-registered / unvetted, tier 3 = verified CERT/ISAC member. Credential issuance is out of scope for the cryptographic protocol itself (it's a governance question — see open issue below) but the *proof* that a contributor holds a given tier is a standard anonymous credential (Groth16 proof of knowledge of a signature under the issuer's key, without revealing which signature).

**Open design question:** who issues tier-3 credentials, and what stops that issuer from becoming the new single point of trust? Current thinking: multiple independent issuers (CERTs, ISACs) whose public keys are all accepted, so no single issuer is a chokepoint — but this needs input before Phase 1 locks it in.

## 3. Proof statement (proof-of-observation)

For a chosen (indicator, epoch) pair, the contributor proves, in zero knowledge:

> "I know a Merkle path from leaf `Poseidon(indicator || epoch || salt)` to a root R that I previously published, AND I hold a valid credential of tier ≥ N, AND this is the first time I've disclosed this (indicator, epoch) pair (nullifier check)."

Public inputs: `indicator`, `epoch`, `R`, `nullifier`, `N` (claimed minimum tier), issuer public key set.
Private inputs: Merkle path, `salt`, credential secret.
