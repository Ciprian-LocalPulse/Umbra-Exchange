...
- **Credential-tier claim is currently unenforced.** As of this writing, `min_tier` is an allocated-but-unconstrained public input in `proof-of-observation` — see that crate's module-level doc comment. Anyone using this circuit before that gadget lands should treat every proof as tier-0 regardless of the claimed `min_tier` value.
